...
clientwin_update2_filled(session_t *ps, MainWin *mw, ClientWin *cw) {
	float scale = cw->paneltype == WINTYPE_WINDOW ? mw->multiplier : 1.0f;
	int width = cw->mini.width > 0 ? cw->mini.width : (int)ceilf(cw->src.width * scale);
	int height = cw->mini.height > 0 ? cw->mini.height : (int)ceilf(cw->src.height * scale);
	width = MAX(width, 1);
	height = MAX(height, 1);
...
void clientwin_prepmove(ClientWin *cw)
{
	...
	session_t *ps = mw->ps;
	float scale = cw->paneltype == WINTYPE_WINDOW ? mw->multiplier : 1.0f;
	int width = MAX(cw->src.width, (int)ceilf(cw->src.width * scale));
	int height = MAX(cw->src.height, (int)ceilf(cw->src.height * scale));
	if (width <= 0 || height <= 0)
		return;
	...
	cw->pixmap = XCreatePixmap(ps->dpy, cw->mini.window,
			width, height, mw->depth);
	XSetWindowBackgroundPixmap(ps->dpy,
			cw->mini.window, cw->pixmap);

	cw->destination = XRenderCreatePicture(ps->dpy,
